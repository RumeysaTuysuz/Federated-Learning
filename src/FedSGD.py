...
def train_fedsgd(client_number, client_ratio_per_round, epochs, test_num=600, batch_size=32):
    ...
    for ep in range(epochs):
        ...
        for client_id in tqdm(random_clients, ascii=True, desc=f"Epoch {ep+1}/{epochs}"):
            ...
            # Get a batch of data (this part is conceptual for FedSGD, the actual update happens below)
            # batch_x, batch_y = client.dataset.train[client_id].next_batch(batch_size)

            # Pass batch_size to train_epoch (even though it runs a full epoch in current Client.py)
